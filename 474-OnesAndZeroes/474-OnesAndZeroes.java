// Last updated: 12/19/2025, 12:00:40 PM
class Solution {
    ...
    public int maxSize(int[][] arr, int idx, int m, int n, int cnt0, int cnt1, int[][][] dp){
        ...
        int noTake = maxSize(arr,idx+1,m,n,cnt0,cnt1,dp);
        int take = 0;
        if (arr[idx][0]+cnt0<=m && arr[idx][1]+cnt1<=n){
            take = 1+maxSize(arr,idx+1,m,n,cnt0+arr[idx][0],cnt1+arr[idx][1],dp);
        }
        return dp[idx][cnt0][cnt1] = Math.max(take,noTake);
    }
}